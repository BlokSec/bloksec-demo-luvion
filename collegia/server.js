...
app.use(log4js.connectLogger(log, {
  ...
  // don't log request for images, css, etc. nor root requests (AWS healt check spams our logs)
  nolog: ['\\/$', '^/css/.', '^/extra-images/.', '^/fonts/.', '^/js/.', '^/images/.', '^/svg/.'],
}));
...
app.use('/extra-images', express.static(path.join(__dirname, 'public', 'extra-images')));
app.use('/svg', express.static(path.join(__dirname, 'public', 'svg')));
app.use('/style.css', express.static(path.join(__dirname, 'public', 'css', 'style.css')));
