...
const makeTokenRevoker = ({ issuer, client_id, client_secret, errorHandler }) => { 
  const revokeEndpoint = `${issuer}/token/revocation`;
...
logout.forceLogoutAndRevoke = function(context) { 
  ...
  return async (req, res, next) => { 
    ...
    const endSessionEndpoint = `${context.options.issuer}/session/end?${querystring.stringify(params)}`;
    return res.redirect(endSessionEndpoint);
  };
};
